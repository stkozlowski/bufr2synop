...
int synop_YYYYMMDDHHmm_to_YYGG ( struct synop_chunks *syn )
{
  ...
  //But syn->e.YYYY, syn->e.MM, syn->e.DD, syn->e.HH, syn->e.mm
  //is here in GMT time (?)
  //If so, conversion of tim to GMT is unnecesseary
  //By the way this UNIX version of conversion does not work properly on Windows
  /*
...
/*
This version recalculates 
syn->e.YYYY, syn->e.MM, syn->e.DD, syn->e.HH, syn->e.mm (assumed as in local time)
to GMT YYGG
...
int synop_YYYYMMDDHHmm_to_YYGG ( struct synop_chunks *syn )
{
  ...
  //Local time (?) date of synop:
  //syn->e.YYYY, syn->e.MM, syn->e.DD, syn->e.HH, syn->e.mm
  //is converted here to GMT YYGG
 ...
 yyyy=atoi(syn->e.YYYY);
 mm=atoi(syn->e.MM);
 dd=atoi(syn->e.DD);
 hh=atoi(syn->e.HH);
 mn=atoi(syn->e.mm);
 ss=0;
 tmloc.tm_year=yyyy-1900; 
 tmloc.tm_mon=mm-1;
 tmloc.tm_mday=dd;
 tmloc.tm_hour=hh;
 tmloc.tm_min=mn;
 tmloc.tm_sec=ss;
 tmloc.tm_isdst=-1;
 //negative value for tm_isdst causes mktime to attempt to find out 
 //whether summer time is in effect for the specified time. 
 tloc=mktime(&tmloc);
 pnewtime=gmtime(&tloc);//get GMT struct tm from tloc
 sprintf ( syn->s0.YY, "%02d", pnewtime->tm_mday );
 sprintf ( syn->s0.GG, "%02d", pnewtime->tm_hour);
